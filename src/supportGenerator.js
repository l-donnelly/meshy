...
// that we don't use the GPU to find the nearest point-mesh intersection,
...
const SupportGenerator = (() => {
    class SupportGenerator {
        ...
        //  epsilon: optional
        generate(params = {}) {
            const angleDegrees = params.angle || 45;
            var resolution = params.resolution || 0.3;
            const layerHeight = params.layerHeight || 0.1;
            const radius = params.radius || 0.1;
            const subdivs = params.subdivs || 16;
            const taperFactor = params.taperFactor || 0.5;
            const radiusFn = params.radiusFn || SupportGenerator.RadiusFunctions.sqrt;
            const radiusFnK = params.radiusFnK || 0.01;
            ...
            const ah = Utils.cycleAxis(axis);
            const av = Utils.cycleAxis(ah);

            // angle in radians
            const angle = (90 - angleDegrees) * Math.PI / 180;
            const minHeight = 0;
            var resolution = resolution;
            const minSupportLength = 3 * radius;

            // used to determine overhangs
            const dotProductCutoff = Math.cos(Math.PI / 2 - angle);
            ...
            const supportFaces = getSupportFaces();

            // rasterize each overhang face set to find sampling points over every set
            const points = samplePoints(supportFaces);

            // create the underlying structure for the support trees
            const supportTrees = buildSupportTrees(points);

            const supportTreeGeometry = new THREE.Geometry();

            const treeWriteParams = {
            ...
            for (const tree of supportTrees) {
                //tree.debug();
            ...
            function getSupportFaces() {
                ...
                const minFaceMax = minHeight + layerHeight / 2;
                const supportFaces = [];
                ...
                for (let f = 0, l = fs.length; f < l; f++) {
                    ...
                    const faceMax = Math.max(a[axis], b[axis], c[axis]);
            ...
            function samplePoints(supportFaces) {
                ...
                const rhmin = boundingBox.min[ah];
                const rvmin = boundingBox.min[av];
                ...
                for (let f = 0, l = supportFaces.length; f < l; f++) {
                    const face = supportFaces[f];

                    Calculate.faceVertices(face, vs, matrixWorld, a, b, c);

                    // bounding box for the face
                    const facebb = Calculate.faceBoundingBox(face, vs, matrixWorld);

                    // normal in world space
                    const normal = face.normal.clone().transformDirection(matrixWorld);

                    // this face's lower bounds in rasterization space
                    const hmin = rhmin + Math.floor((facebb.min[ah] - rhmin) / resolution) * resolution;
                    const vmin = rvmin + Math.floor((facebb.min[av] - rvmin) / resolution) * resolution;
                    // this face's upper bounds in rasterization space
                    const hmax = rhmin + Math.ceil((facebb.max[ah] - rhmin) / resolution) * resolution;
                    const vmax = rvmin + Math.ceil((facebb.max[av] - rvmin) / resolution) * resolution;
                    ...
                    for (let ph = hmin; ph < hmax; ph += resolution) {
                        for (let pv = vmin; pv < vmax; pv += resolution) {
                            ...
                            if (Utils.pointInsideTriangle(pt, b, a, c, axis, epsilon)) {
                                points.push({
                                    v: Utils.projectToPlaneOnAxis(pt, a, normal, axis),
                                    normal
                                });
                            }
                        }
            ...
            function buildSupportTrees(points) {
                ...
                const pqComparator = (a, b) => nodes[b].v[axis] - nodes[a].v[axis];
                ...
                for (var pi = 0; pi < points.length; pi++) {
                    ...
                    const startNode = new SupportTreeNode(v);
                    ...
                    // along the normal
                    const raycastNormal = octree.raycast(ray.set(v, normal));
                    const nv = v.clone().addScaledVector(normal, minSupportLength);
                    ...
                    // else, connect a new support node to the start node
                        ...
                        const newNode = new SupportTreeNode(nv, startNode);
                ...
                while (pq.length > 0) {
                    ...
                    for (const qi of activeIndices) {
                        var q = nodes[qi];
                        const ixn = Utils.coneConeIntersection(p.v, q.v, angle, axis);

                        // if valid intersection and it's inside the mesh boundary
                        if (ixn && (ixn[axis] - minHeight > radius)) {
                            const pidist = p.v.distanceTo(ixn);
                            const qidist = q.v.distanceTo(ixn);
                    ...
                    const raycastDown = octree.raycast(ray.set(p.v, down));
                    ...
                    // straight down
                    const pointDown = new THREE.Vector3();
                    let distanceDown = 0;
                    ...
                    if (intersection) {
                        const d = intersection.clone().sub(p.v).normalize();
                        // cast a ray from p to the intersection
                        const raycastP = octree.raycast(ray.set(p.v, d));
                        ...
                        // no obstacle for p's strut
                            ...
                            const raycastIntersectionDown = octree.raycast(ray.set(intersection, down));
                    ...
                    const noTaper = target.equals(pointDown) && !raycastDown;
    ...
    SupportGenerator.RadiusFunctions = {
        constant(r, w, k) { return r; },
        sqrt(r, w, k) { return r + k * Math.sqrt(w); }
    };
    ...
    class SupportTreeNode {
        ...
        writeToGeometry(params) {
            if (!this.isRoot()) return null;

            params = params || {};
            let subdivs = params.subdivs;
        ...
        makeProfiles(params) {
            ...
            const vertices = params.geo.vertices;

            const isRoot = this.isRoot();
            const isLeaf = this.isLeaf();
            const isElbowJoint = this.isElbowJoint();

            const radius = params.radius;
            const subdivs = params.subdivs;
            const endOffsetFactor = params.endOffsetFactor;
            ...
            if (isRoot || isLeaf) {
                ...
                const vn = n.v.clone().sub(this.v).normalize();

                // point where the profile center will go
                const endOffset = this.noTaper ? 0 : -endOffsetFactor * r;
                const p = this.v.clone().addScaledVector(vn, endOffset);

                // two axes orthogonal to strut axis
                var b = Utils.orthogonalVector(vn).normalize();
                ...
                const s = v0.clone().add(vs);
                var b = Utils.equal(s.length(), 0) ? Utils.orthogonalVector(v0) : s;
                b.normalize();

                // half-angle between the struts
                const ha = Utils.acos(v0.dot(vs)) / 2;

                // distance from center to the farthest intersection of the struts
                const m = r / Math.sin(ha);
                ...
                const v1 = this.b1.v.clone().sub(this.v).normalize();
                var vs = this.source.v.clone().sub(this.v).normalize();

                // sums of adjacent strut vectors
                const sm01 = v0.clone().add(v1);
                const sm0s = v0.clone().add(vs);
                const sm1s = v1.clone().add(vs);
                ...
                const b01 = Utils.equal(sm01.length(), 0) ? Utils.projectOut(vs, v0).negate() : sm01;
                const b0s = Utils.equal(sm0s.length(), 0) ? Utils.projectOut(v1, vs).negate() : sm0s;
                const b1s = Utils.equal(sm1s.length(), 0) ? Utils.projectOut(v0, v1).negate() : sm1s;
                // normalize bisectors
                b01.normalize();
                b0s.normalize();
                b1s.normalize();
                ...
                const mio = r / Math.sqrt(1 - dot*dot);
                ...
                const d01 = ihat.dot(b01);
                const d0s = ihat.dot(b0s);
                const d1s = ihat.dot(b1s);
                ...
                const s01 = Utils.acos(mio * d01 / m01);
                const s0s = Utils.acos(mio * d0s / m0s);
                const s1s = Utils.acos(mio * d1s / m1s);
                ...
                const c01 = Utils.projectOut(ihat, b01).normalize();
                const c0s = Utils.projectOut(ihat, b0s).normalize();
                const c1s = Utils.projectOut(ihat, b1s).normalize();

                // minor axis magnitudes
                const n01 = mio * Math.sqrt(1 - d01*d01) / Math.sin(s01);
                const n0s = mio * Math.sqrt(1 - d0s*d0s) / Math.sin(s0s);
                const n1s = mio * Math.sqrt(1 - d1s*d1s) / Math.sin(s1s);

                // put the calculated points into the geometry

                // indices of inward and outward vertices
                const inidx = vertices.length;
                ...
                const scount = (subdivs - 2) / 2;
                const scount1 = scount + 1;

                // start indices of each arc
                const s01idx = vertices.length;
                const s0sidx = s01idx + scount;
                const s1sidx = s0sidx + scount;
                ...
                let dir = ihat.clone().cross(vs).dot(v0);
                if (Utils.equal(dir, 0)) dir = -ihat.clone().cross(vs).dot(v1);

                // s strut left and right indices
                const idxsL = dir > 0 ? s0sidx : s1sidx;
                const idxsR = dir > 0 ? s1sidx : s0sidx;
                // 0 strut left and right indices
                const idx0L = dir > 0 ? s0sidx : s01idx;
                const idx0R = dir > 0 ? s01idx : s0sidx;
                // 1 strut left and right indices
                const idx1L = dir > 0 ? s01idx : s1sidx;
                const idx1R = dir > 0 ? s1sidx : s01idx;
        ...
        connectToBranch(n, params) {
            ...
            const sp = (n === this.b0) ? this.p0 : this.p1;
            const tp = n.ps;
            ...
            for (var ii = 0; ii < subdivs; ii++) {
                const a = tp[(tidx + ii) % subdivs];
                const b = tp[(tidx + ii + 1) % subdivs];
                const c = sp[ii];
                const d = sp[(ii + 1) % subdivs];
        ...
        makeCap(params) {
            ...
            const iincr = this.isRoot() ? subdivs - 1 : 1;

            // index of center vertex
            const pc = p[subdivs];

            // write faces
            for (let ii = 0; ii < subdivs; ii++) {
        ...
        offsetLimit(radius) {
            ...
            const vn = this.v.clone().sub(n.v).normalize();
            ...
            if (a) {
                ...
                const va = a.v.clone().sub(n.v).normalize();

                // bisector between n and a
                const bna = vn.clone().add(va).normalize();

                // dot product between vn and a-n bisector
                const dna = vn.dot(bna);
            ...
            if (b) {
                ...
                const vb = b.v.clone().sub(n.v).normalize();

                // bisector between n and b
                const bnb = vn.clone().add(vb).normalize();

                // dot product between vn and b-n bisector
                const dnb = vn.dot(bnb);

...
const Measurement = (() => {
    ...
    Object.assign(Measurement.prototype, {
        ...
        start(params) {
            ...
            const pparams = { name: `measurementMarkerPrimary${this.uuid}` };
            const sparams = { name: `measurementMarkerSecondary${this.uuid}` };
        ...
        dispose() {
            ...
            const namePrimary = `measurementMarkerPrimary${this.uuid}`;
            const nameSecondary = `measurementMarkerSecondary${this.uuid}`;
            const namePreview = `measurementMarkerPreview${this.uuid}`;
        ...
        calculate() {
            ...
            if (type === Measurement.Types.length) {
                ...
                const d10 = new Vector3().subVectors(p0, p1).normalize();
                const d12 = new Vector3().subVectors(p2, p1).normalize();
                const dot = d10.dot(d12);

                this.result.angle = Utils.acos(dot);
                ...
                let contours = Calculate.crossSection(plane, this.pointer.objects, this.params.splitContours);
                ...
                if (this.params.convexHull) {
                    const hull = Calculate.planarConvexHull(plane, segments);
        ...
        positionPrimaryMarkers() {
            if (this.ptype === Markers.Types.sphere) {
                for (let m = 0; m < this.pnum; m++) {
                    const pos = this.params.p[m];
        ...
        positionSecondaryMarkers() {
            if (this.stype === Markers.Types.line) {
                for (let m = 0; m < this.snum; m++) {
                    const ps = this.params.p[(this.pidx + m) % this.pnum];
                    const pt = this.params.p[(this.pidx + 1 + m) % this.pnum];
                ...
                if (this.result.ready) {
                    ...
                    const radius = this.result.radius;
        ...
        positionPreviewMarker() {
            ...
            const p0 = this.params.p[0];
            const p1 = this.params.p[1];
            const p2 = this.params.p[2];

            // compute the circle parameters from three points
            const circle = Calculate.circleFromThreePoints(p0, p1, p2);
        ...
        scaleFromPoint(factor, point) {
            ...
            const f = factor.isVector3 ? factor.x : factor;

...
export const infillLinear = function(min, max, spacing, angle, parity) {
        let context = min.context;
        ...
        const d = new Vector(context).setUnitVector("v").rotate(-angle);
        const dh = d.h;
        const dv = d.v;

        // vector by which the line is shifted at every iteration
        const shift = MCGMath.orthogonalRightVector(d, spacing);

        // top left and top right corners
        const topleft = new Vector(context, min.h, max.v);
        const topright = max;

        // lines have positive slope
        const spos = angle < pi_2;

        // if 0-90 degrees CW from the vertical, start from top-left corner;
        // if 90-180, start from top-right corner
        const p = spos ? new Vector(context, min.h, max.v) : max.clone();

        // horizontal/vertical values the line will cross first/second
        const h1 = min.h;

        const v1 = spos ? min.v : max.v;
        const h2 = max.h;
        const v2 = spos ? max.v : min.v;

        const lines = new SegmentSet(context);
        ...
        while (1) {
            ...
            const th1 = dh !== 0 ? ((h1 - p.h) / dh) : -Infinity;
            const tv1 = dv !== 0 ? ((v1 - p.v) / dv) : -Infinity;
            const t1 = Math.max(th1, tv1);

            const th2 = dh !== 0 ? ((h2 - p.h) / dh) : Infinity;
            const tv2 = dv !== 0 ? ((v2 - p.v) / dv) : Infinity;
            const t2 = Math.min(th2, tv2);

            // line is outside the box, so subsequent lines will be outside too
            if (t1 >= t2) break;

            // calculate intersection points
            const p1 = p.clone().addScaledVector(d, t1);
            const p2 = p.clone().addScaledVector(d, t2);
...
export const infillHex = function(min, {h, v}, spacing, linewidth, parity) {
        let context = min.context;
        spacing = spacing || context.p;
        ...
        const dh = spacing * sqrt3 / 2;
        const dv = spacing / 2;

        const vertical = new Vector(context, 0, spacing);
        ...
        if (parity === 0) {
            ...
            const right = new Vector(context, dh, dv);
            const left = new Vector(context, -dh, dv);
            ...
            while (p.h < h) {
                ...
                while (p.v < v) {
                    ...
                    const p3 = p2.clone().add(vertical);
                    const p4 = p3.clone().add(even ? left : right);
        ...
        // if parity !== 0, go across in a zigzag pattern
            ...
            const dhz = dh + linewidth;
            const dvz = dhz / sqrt3;

            // vertical shift in start point for even and odd iterations
            const vshifteven = dv*2 + spacing + (dvz - dv);
            const vshiftodd = (spacing*2 + dv*2) - vshifteven;

            const up = new Vector(context, dhz, dvz);
            const down = new Vector(context, dhz, -dvz);

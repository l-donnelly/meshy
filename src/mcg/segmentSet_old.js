class SegmentSet {
    ...
    addSegment(v1, v2, normal) {
        if (coincident(v1, v2, this.epsilon)) return;

        let segment;
        const dot = this.up.clone().cross(normal).dot(v2.clone().sub(v1));
    ...
    unify() {
        ...
        const efactory = new SweepEventFactory();

        const drawEvents = false;
        let printEvents = true;
        const drawSourceSegments = false;
        const incrHeight = false;

        // priority queue storing events from left to right
        const sweepEvents = new PriorityQueue({
        ...
        const status = new RBTree(rbtComparator);

        let o = 1.0;
        let prev = null;
        while (sweepEvents.length > 0) {
            const event = sweepEvents.dequeue();
        ...
        function pqComparator({id, v, isLeft, slope}, {id, v, isLeft, slope}) {
            if (id === id) return 0;

            const va = v;
            const vb = v;

            const hcomp = compare(va[ah], vb[ah], epsilon);

            if (hcomp !== 0) return hcomp;
            else {
                const vcomp = compare(va[av], vb[av], epsilon);

                if (vcomp !== 0) return vcomp;
                else {
                    if (!isLeft && isLeft) return -1;
                    else if (isLeft && !isLeft) return 1;
                    else {
                        const scomp = compare(slope, slope, epsilon);

                        if (scomp !== 0) return scomp;
                        else {
                            return Math.sign(id - id);
                        }
                    }
                }
            }
        ...
        function rbtComparator(a, b) {
            ...
            const vcomp = vcompare(a, b);

            if (vcomp !== 0) return vcomp;
            else {
                const scomp = compare(a.slope, b.slope, epsilon);
        ...
        function addSegmentEvents(segment) {
            ...
            // make events
            const event1 = efactory.create(v1);
            const event2 = efactory.create(v2);

            // link events to each other
            event1.twin = event2;
            event2.twin = event1;

            const vertical = equal(v1[ah], v2[ah], epsilon);
            const dir = vertical ? (v1[av] < v2[av]) : (v1[ah] < v2[ah]);
            ...
            const slope = vertical ? Infinity : (v2[av] - v1[av]) / (v2[ah] - v1[ah]);
        ...
        function handleEventOverlap(a, b) {
            ...
            // else horizontal)
            const caxis = vertical ? av : ah;

            const lcomp = compare(va[caxis], vb[caxis], epsilon);
            const rcomp = compare(vta[caxis], vtb[caxis], epsilon);

            // there will be up to two split points, one on the left, another on the
            // right

            // first and second left events
            const lf = lcomp > 0 ? b : a;
            const ls = lcomp > 0 ? a : b;
            // vertex at which lf will be split
            const vl = lcomp > 0 ? va : vb;

            // first and second right events
            const rf = rcomp > 0 ? tb : ta;
            const rs = rcomp > 0 ? ta : tb;
            // vertex at which rs's twin will be split
            const vr = rcomp > 0 ? vtb : vta;
        ...
        function handleEventIntersection(a, b) {
            ...
            const vi = computeEventIntersection(a, b);
        ...
        function eventSplit(e, vi) {
            ...
            const ei = efactory.clone(te, vi);
            const ite = efactory.clone(e, vi);
        ...
        function eventValid({contributing, depthAbove, depthBelow}) {
        ...
        function eventColor({isLeft, twin, contributing}) {
            if (!isLeft) {
        ...
        function statusDraw(incr = 0, force) {
            const it = status.iterator();
            let e;
            while ((e = it.next()) !== null) {
...
class SweepEvent {
    ...
    setDepthFromBelow(eventBelow) {
        const depthBelow = eventBelow !== null ? eventBelow.depthAbove : 0;
    ...
    endpointsCoincident({v, twin}, epsilon) {
        if (coincident(this.v, v, epsilon)) return true;
        if (coincident(this.twin.v, twin.v, epsilon)) return true;
...
class SweepEventFactory {
    constructor() {
        this.id = 0;
    }

    create(v) {
        const e = new SweepEvent(v);
        e.id = this.id++;

...
const Octree = (() => {
    class Octree {
        constructor(mesh, params) {
            ...
            const overflow = params.overflow || 0.00001;
            const origin = params.origin || this.min.clone().subScalar(overflow / 2);
            const size = params.size || Utils.vector3MaxElement(this.max.clone().sub(this.min)) + overflow;
            let depth;
            ...
            if (params.hasOwnProperty("depth")) {
                ...
                const vsize = this.max.clone().sub(this.min);
                const vratio = (size * size * size) / (vsize.x * vsize.y * vsize.z);
            ...
            for (let f = 0, l = faces.length; f < l; f++) this.addFace(f);
        ...
        addFace(i) {
            ...
            this.node.addFace({
                    verts: Utils.faceGetVerts(face, this.vertices),
                    normal: face.normal
                },
                i);
        }
        ...
        visualize(scene, drawLines, depthLimit) {
            if (!scene) return;
            this.unvisualize();

            const outlineGeo = new THREE.Geometry();
        ...
        visualizeBorderEdges(scene) {
            if (!scene) return;

            const borderGeo = new THREE.Geometry();
    ...
    class TreeNode {
        ...
        calculateEdgeIntersections(faces, vertices) {
            if (!faces || !vertices) return;
            ...
            if (depth==0) {
                ...
                const s = this.nodeVertices();
                for (var i=0; i<this.children.length; i++) {
                    const face = faces[this.children[i]];
                    ...
                    for (let j=0; j<12; j++) {
                        const edgeIndices = this.edgeIndices[j];
                        const s1 = s[edgeIndices[0]];
                        const s2 = s[edgeIndices[1]];
        ...
        // of the tree
        visualize(geo, drawLines, depthLimit) {
            if (this.depth==0 || (depthLimit!==undefined && this.depth==depthLimit)) {
                if (drawLines) {
                    const v = this.nodeVertices();
                    ...
                    const center = this.origin.clone().addScalar(this.size/2);
        ...
        visualizeBorderEdges(geo) {
            if (this.depth==0) {
                ...
                for (var i=0; i<6; i++) {
                    const edges = this.faceEdgeIndices[i];
                    ...
                    if (total&1 != 0) {
                        for (var j=0; j<4; j++) {
                            const edgeIndices = this.edgeIndices[edges[j]];
    ...
    class Raycaster {
        ...
        castRay(node, ray, mesh) {
            ...
            const t0 = node.origin.clone().sub(p).divide(d);
            const t1 = node.origin.clone().addScalar(node.size).sub(p).divide(d);
            ...
            this.rayEnd = p.clone().addScaledVector(d, Utils.vector3MinElement(t1));
        ...
        hitTest(faceIdx) {
            ...
            const internal = this.type === raycasterTypes.internal;
            const external = this.type === raycasterTypes.external;
            const dot = face.normal.dot(this.d);
            ...
            let b;
            let c;
            if (internal) [a, b, c] = Utils.faceGetVerts(face, this.vertices);
            else [b, a, c] = Utils.faceGetVerts(face, this.vertices);

            // get the intersection point of the face with the ray
            const point = triSegmentIntersection(a, b, c, this.p, this.rayEnd);
        ...
        castRayProc(node, t0, t1) {
            ...
            if (node.depth==0) {
                ...
                for (let i=0; i<children.length; i++) {
                    ...
                    const faceIdx = children[i];

                    // get the intersection point in world space
                    const point = this.hitTest(faceIdx);

                    // if point exists, need to set the intersection object
                    if (point) {
                        const pointWorld = point.clone().applyMatrix4(this.mesh.matrixWorld);
                        const originWorld = this.p.clone().applyMatrix4(this.mesh.matrixWorld);
                        const distanceWorld = originWorld.distanceTo(pointWorld);
            ...
            // each child node the ray crosses (in the order it crosses them)
                ...
                const tm = t0.clone().add(t1).divideScalar(2.0);
                // adjusting for the case of axis-asligned rays
                const nodeCenter = node.origin.clone().addScalar(node.size/2.0);
                if (Utils.isInfinite(t0.x)) tm.x = p.x<nodeCenter.x ? Infinity : -Infinity;
                if (Utils.isInfinite(t0.y)) tm.y = p.y<nodeCenter.y ? Infinity : -Infinity;
                if (Utils.isInfinite(t0.z)) tm.z = p.z<nodeCenter.z ? Infinity : -Infinity;
                ...
                const axis = Utils.vector3ArgMax(t0);
                ...
                while (currentChildIdx>-1) {
                    const child = node.children[currentChildIdx];
                    const childParams = this.getChildParams(currentChildIdx, t0, tm, t1);
        ...
        getChildParams(idx, t0, {x, y, z}, t1) {
            const dir = this.dir;
            ...
            // if not axis-aligned on x
                ...
                const xnear = (!xmask && dir.x>0) || (xmask && dir.x<0);
            ...
            if (dir.y==0) {
                ...
                const ynear = (!ymask && dir.y>0) || (ymask && dir.y<0);
                if (ynear) t1c.y = y;
                else t0c.y = y;
            }
            ...
            if (dir.z==0) {
                ...
                const znear = (!zmask && dir.z>0) || (zmask && dir.z<0);
                if (znear) t1c.z = z;
                else t0c.z = z;
            }
        ...
        getNextChild(idx, t1) {
            ...
            const exitAxis = Utils.vector3ArgMin(t1);
    ...
    function cubeIntersectsTri(o, s, {verts, normal}) {
        const v0 = verts[0];
        const v1 = verts[1];
        const v2 = verts[2];
        let min;
        ...
        if (!cubeIntersectsPlane(o, s, verts[0], normal)) return false;
    ...
    function axisCrossEdgeIntersection({oy, oz, ox}, s, axisIdx, {fz, fy, fx}, va, vb) {
        let pa;
        let pb;
        let min;
        let max;
        ...
        if (axisIdx===0 && (fz*fz + fy*fy)>epsilon) {
        ...
        else if (axisIdx==1 && (fz*fz + fx*fx)>epsilon) {
            c.set(fz, 0, -fx);
            pa = c.dot(va), pb = c.dot(vb);
            if (pa>pb) { min = pb; max = pa; }
            else { min = pa; max = pb; }
        ...
        else if (axisIdx==2 && (fy*fy + fx*fx)>epsilon) {
            c.set(-fy, fx, 0);
            pa = c.dot(va), pb = c.dot(vb);
            if (pa>pb) { min = pb; max = pa; }
            else { min = pa; max = pb; }
    ...
    function cubeIntersectsPlane(o, s, v, n) {
        const vmin = new THREE.Vector3().subVectors(o, v);
        const vmax = new THREE.Vector3().subVectors(o, v);

const AxisWidget = function (sourceCamera) {
    ...
    const cubeMat = new THREE.MeshPhongMaterial({color: 0xbbbbbb, shininess: 10});
    const cubeMesh = new THREE.Mesh(cubeGeo, cubeMat);
    this.scene.add(cubeMesh);

    const _this = this;
    const fontLoader = new THREE.FontLoader();
    fontLoader.load('./src/helvetiker_regular.typeface.json', font => {
        const params = {
            font,
            size: 7,
            height: 1
        };
        const dist = _this.size/2 + 1;
        const geos = [

...
const Repair = (() => {
    ...
    function generatePatchGeometry({geometry}, p = 1e5) {
        // get the hash table detailing vertex adjacency
        const adjacencyMap = generateAdjacencyMap(
            geometry.vertices,
            geometry.faces,
            p, true, true
        );
        ...
        const borderMap = generateBorderMap(adjacencyMap);

        // check for empty border map; if properties exist, then holes exist
        if (Utils.objectIsEmpty(borderMap)) {
            return null;
        }
        ...
        while (true) {
            ...
            while (true) {
                ...
                const hash = Utils.vertexHash(current, p);
                ...
                if (previous==null) {
                    ...
                    const currentAdjacentData = adjacencyMap[hash];
                ...
                //    that borders the same hole
                    ...
                    if (data.numHoles===1) {
                        ...
                        // will be 0
                        const prevEdge = edges[neighbors.indexOf(previous)];
                        // orthogonal to both prevEdge and normal; use this to test for angles
                        // greater than pi
                        const orthogonalVector = prevEdge.clone().cross(normal);
        ...
        for (let c=0; c<borderCycles.length; c++) {
            var cycle = borderCycles[c].slice();
            const normals = borderCycleNormals[c];

            var n = cycle.length;
            const originalCycleLength = n;
            let originalCyclePathLength = 0;
            const originalFaceCount = patchFaces.length;
            ...
            while (cycle.length>3) {
                ...
                if (count%originalCycleLength==0) {
                    const newPathLength = edges.reduce((acc, x) => acc+x.length(), 0);
                ...
                var v = cycle[idx];
                const vprev = cycle[prevIdx];
                const vnext = cycle[nextIdx];

                // indices into the patch vertex array
                const patchvidx = Utils.vertexMapIdx(patchVertexMap, v, patchVertices, p);
                const patchprevidx = Utils.vertexMapIdx(patchVertexMap, vprev, patchVertices, p);
                const patchnextidx = Utils.vertexMapIdx(patchVertexMap, vnext, patchVertices, p);

                // edges from v to next and from v to prev
                const enext = edges[idx];
                const eprev = edges[prevIdx].clone().multiplyScalar(-1);

                const centerVector = center.clone().sub(v);
                ...
                if (angle < 1.308996939) { // if angle < 75 degrees
                    ...
                    const prevlen = eprev.length();

                    const nextlen = enext.length();
                    ...
                    const axis = enext.clone().cross(eprev).normalize();
                ...
                if (newVerts.length===0) {
                    ...
                    var patchv1idx = Utils.vertexMapIdx(patchVertexMap, v1, patchVertices, p);
                    ...
                    var patchv1idx = Utils.vertexMapIdx(patchVertexMap, v1, patchVertices, p);
                    const patchv2idx = Utils.vertexMapIdx(patchVertexMap, v2, patchVertices, p);

                    // new edges
                    var e1 = v1.clone().sub(v);
                    const e2 = v2.clone().sub(v);
            ...
            if (cycle.length===3) {
                var face = new THREE.Face3();
                face.a = Utils.vertexMapIdx(patchVertexMap, cycle[0], patchVertices, p);
                face.b = Utils.vertexMapIdx(patchVertexMap, cycle[2], patchVertices, p);
                face.c = Utils.vertexMapIdx(patchVertexMap, cycle[1], patchVertices, p);
                const e01 = cycle[1].clone().sub(cycle[0]);
                const e02 = cycle[2].clone().sub(cycle[0]);
            ...
            const vertices = patchGeometry.vertices;
            const faces = patchGeometry.faces.slice(originalFaceCount);
            const patchAdjacencyMap = generateAdjacencyMap(vertices, faces, p);
            ...
            for (var key in patchAdjacencyMap) {
                if (cycle.includes(patchAdjacencyMap[key].vertex)) {
                    ...
                    data.copyNeighbors = data.neighbors.map(x => x.clone());
                }
            }
            ...
            for (var i=0; i<numIterations; i++) {
                ...
                for (var key in patchAdjacencyMap) {
                    ...
                    const sum = neighbors.reduce((acc, x) => acc.add(x), new THREE.Vector3());
                ...
                for (var key in patchAdjacencyMap) {
                    var data = patchAdjacencyMap[key];
                    for (let j=0; j<data.neighbors.length; j++) {
        ...
        function calculateAngleFromEdges(idx, edges, cycle, normals, n) {
            ...
            const e1 = edges[prevIdx].clone().normalize().multiplyScalar(-1);
            const e2 = edges[idx].clone().normalize();
            let angle = Math.acos(e1.dot(e2));
    ...
    function generateAdjacencyMap(vertices, faces, p, storeWindingOrder, storeNormal) {
        ...
        for (let f=0; f<faces.length; f++) {
            const face = faces[f];
            const faceVerts = Utils.faceGetVerts(face, vertices);

            // for each vertex in the face
            for (let v=0; v<3; v++) {
                const vertex = faceVerts[v];
                const hash = Utils.vertexHash(vertex, p);

                // the other two vertices for the face; we will add these to adjacencyMap
                const vertex1 = faceVerts[(v+1)%3];
                const vertex2 = faceVerts[(v+2)%3];
        ...
        function addAdjacentVertex(vertex, data, windingOrder) {
            ...
            const idx = data.neighbors.indexOf(vertex);
    ...
    function generateBorderMap(adjacencyMap) {
        ...
        for (const key in adjacencyMap) {
            let edgeVertex = false;
            const data = adjacencyMap[key];
            let singleNeighborCount = 0;

            for (let c=0; c<data.windingOrder.length; c++) {
            ...
            if (edgeVertex) {
                const neighbors = [];
                for (let v=0; v<data.neighbors.length; v++) {

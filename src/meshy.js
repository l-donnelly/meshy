...
class Meshy {
    ...
    generateUI() {
        ...
        const importSettingsFolder = this.gui.addFolder("Import Settings", "Defaults for the imported mesh.");
        ...
        const exportFolder = this.gui.addFolder("Export", "Mesh export.");
        this.filename = "meshy";
        ...
        const settingsFolder = this.gui.addFolder("Settings", "Settings for computation.");
        ...
        const displayFolder = this.gui.addFolder("Display", "Mesh and build volume display settings.");
        ...
        const thicknessFolder = this.gui.addFolder("Mesh Thickness", "Visualize approximate local mesh thickness.");
        ...
        const repairFolder = this.gui.addFolder("Repair", "Repair missing polygons.");
    ...
    buildEditFolder() {
        ...
        const translateFolder = this.editFolder.addFolder("Translate", "Translate the mesh on a given axis.");
        ...
        const rotateFolder = this.editFolder.addFolder("Rotate",
        ...
        const scaleFolder = this.editFolder.addFolder("Scale", "Scale the mesh by given criteria.");
        ...
        const ringSizeFolder = scaleFolder.addFolder("Scale To Ring Size",
        ...
        this.ringSize = 0;
        ringSizeFolder.add(this, "ringSize", Utils.ringSizes).name("2. Ring size")
        ...
        const mirrorFolder = this.editFolder.addFolder("Mirror", "Mirror the mesh on a given axis in object space.");
        ...
        const floorFolder = this.editFolder.addFolder("Floor", "Floor the mesh on a given axis.");
        ...
        const centerFolder = this.editFolder.addFolder("Center", "Center the mesh on a given axis in the build volume.");
    ...
    buildSupportSliceFolder() {
        const supportSliceFolder = this.supportSliceFolder;
    ...
    buildSliceDisplayFolder(folder) {
    ...
    buildSliceFolder(folder) {
        this.clearFolder(folder);

        if (this.sliceModeOn) {
            const maxLevel = this.model.getMaxSliceLevel();
            const minLevel = this.model.getMinSliceLevel();
    ...
    setFolderDisplayPrecision({__controllers, __folders}) {
        ...
        for (const fkey in __folders) {
            this.setFolderDisplayPrecision(__folders[fkey]);
        }
    }
    ...
    initViewport() {
        ...
        function init() {
            ...
            _this.debug = new Debug(_this.scene); // todo: remove
        ...
        function render() {
            ...
            _this.forEachMeasurement(({measurement}) => {
                measurement.updateFromCamera(_this.camera);
            });
            // update axis widget
    ...
    createModel(geometry, filename) {
        ...
        if (this.units !== this.importUnits) {
            ...
            const convert = Units.getConverterV3(this.importUnits, this.units);

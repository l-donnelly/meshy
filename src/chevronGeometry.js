const ChevronBufferGeometry = (() => {
    class ChevronBufferGeometry extends THREE.BufferGeometry {
        constructor(width, height, thickness, insetHeight) {
            ...
            const topWallLength = Math.sqrt(halfWidth * halfWidth + height * height);
            const insetWidth = insetHeight === 0 ? 0 : insetHeight * halfWidth / height;
            const insetWallLength = Math.sqrt(insetHeight * insetHeight + insetWidth * insetWidth);
            const botWallLength = halfWidth - insetWidth;
            const perimeterHalfLength = topWallLength + botWallLength + insetWallLength;
            ...
            function generateWall(right) {
                ...
                let perimeterPos = right ? perimeterHalfLength * 2 : 0;
            ...
            function generateCap(top) {
                const sign = top ? 1 : -1;
                const normal = new THREE.Vector3(0, sign, 0);
                const vertex = new THREE.Vector3();
                const y = halfThickness * sign;
